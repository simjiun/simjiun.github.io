...
- Fuzzing의 최종 목적은 입력 처리 과정에서 발생하는 버그를 자동으로 찾는 것
...
| 종류  | 설명 |
| --- | --- |
| `fast` | 드물게 실행된 경로에 더 많은 에너지를 부여 |
| `explore` (기본값) | 모든 경로를 균등하게 탐색 |
| `exploit` | 최근에 새 경로를 발견한 seed에 집중 |
| `coe` | 너무 자주 선택된 seed를 차단 |
| `mmopt` | runtime 가중을 줄이고 최근 queue 항목을 더 우선하는 실험적 스케줄 |
...
| mmopt | `-p mmopt` | runtime 가중 축소 + 최근 queue 항목 우선 | 새 queue 중심 탐색 |
